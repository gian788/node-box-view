...
BoxViewer.prototype.fileTypes = ['.pdf', '.doc', '.docx', '.ppt', '.pptx', '.xls', '.xlsx', '.txt',
								 '.py', '.js', '.xml', '.css', '.md', '.pl', '.c', '.m', '.json'];
 ...
 * @param destPath 		String 		png destination file path, including file name and extension
 * @param callback	Function 	callback function
 */
	...
	var req = request.get({
		...
		.on('response', function(res){
			...
			if(res.statusCode == 202)
	  			err = new Error('The thumbnail isn’t available yet. Retry after ' + res.headers['retry-after'] + ' seconds');
